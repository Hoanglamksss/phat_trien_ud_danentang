...
export default function CarDetailsScreen() {
  ...
  useEffect(() => {
    if (!carId) {
      if (!hasNavigatedBack) {
          Alert.alert('Lỗi', 'ID xe không hợp lệ.', [{text: 'OK', onPress: () => router.replace('/(protected)/') }]);
    ...
    const fetchCar = async () => {
      try {
        ...
        if (!hasNavigatedBack) {
            Alert.alert('Không tìm thấy thông tin xe.', [{text: 'OK', onPress: () => router.replace('/(protected)/') }]); 
            setHasNavigatedBack(true);
        }
      } finally {
        setLoading(false);
  ...
  const handleConfirmBooking = () => {
    if (totalPrice <= 0 || !car || !startDate || !endDate || !isValid(startDate) || !isValid(endDate) || endDate <= startDate) {
      Alert.alert('Vui lòng chọn thời gian bắt đầu và kết thúc hợp lệ (kết thúc phải sau bắt đầu).');
      return;
    }
